"use client";

import { useState } from "react";
import type { AppUser, CatalogoOperativo } from "@/types/catalogos";

export function OperativoForm({
  action,
  initialValue,
  userOptions = [],
}: {
  action: (formData: FormData) => Promise<void>;
  initialValue?: CatalogoOperativo;
  userOptions?: AppUser[];
}) {
  const [pending, setPending] = useState(false);

  return (
    <form
      action={async (formData) => {
        setPending(true);
        try {
          await action(formData);
        } finally {
          setPending(false);
        }
      }}
      className="space-y-4"
    >
      <div>
        <label htmlFor="nombre_operativo" className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">
          Nombre operativo
        </label>
        <input
          id="nombre_operativo"
          name="nombre_operativo"
          required
          defaultValue={initialValue?.nombre_operativo ?? ""}
          className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm focus:border-slate-500 focus:outline-none dark:border-slate-700 dark:bg-slate-800"
        />
      </div>

      <div>
        <label htmlFor="user_id" className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">
          Usuario asociado
        </label>
        <select
          id="user_id"
          name="user_id"
          defaultValue={initialValue?.user_id ?? ""}
          className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm focus:border-slate-500 focus:outline-none dark:border-slate-700 dark:bg-slate-800"
        >
          <option value="">— Sin usuario —</option>
          {userOptions.map((u) => (
            <option key={u.id} value={u.id}>
              {u.email}
            </option>
          ))}
        </select>
      </div>

      <div className="flex items-center gap-2">
        <input
          id="activo"
          name="activo"
          type="checkbox"
          defaultChecked={initialValue?.activo ?? true}
          className="h-4 w-4 rounded border-slate-300 dark:border-slate-600"
        />
        <label htmlFor="activo" className="text-sm font-medium text-slate-700 dark:text-slate-300">
          Activo
        </label>
      </div>

      <div className="flex justify-end gap-3 border-t border-slate-200 pt-4 dark:border-slate-800">
        <a
          href="/catalogos/operativos"
          className="rounded-md border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
        >
          Cancelar
        </a>
        <button
          type="submit"
          disabled={pending}
          className="rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-700 disabled:opacity-50 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-slate-300"
        >
          {pending ? "Guardando..." : "Guardar"}
        </button>
      </div>
    </form>
  );
}
