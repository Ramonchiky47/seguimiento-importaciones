...
import { FIELD_LABELS, FIELD_SECTIONS } from "@/types/exportacion";

function formatValue(row: Exportacion, field: string): string {
  const value = (row as unknown as Record<string, string | number | null>)[field];
  if (value === null || value === undefined || value === "") return "(sin datos)";
  return String(value);
}

export function buildExportacionPdf(row: Exportacion, incidencias: Incidencia[] = []): jsPDF {
  ...
  const heading = row.booking || `Registro de exportación #${row.id}`;
