...
export default async function VendedoresPage() {
  ...
  const rows = (data ?? []) as CatalogoVendedor[];
      ...
      <header className="border-b border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
        <div className="mx-auto flex max-w-3xl items-center justify-between px-6 py-4">
          <div>
            ...
            <h1 className="text-lg font-semibold text-slate-900 dark:text-slate-50">Vendedores</h1>
          </div>
          <Link
            href="/catalogos/vendedores/nuevo"
        ...
        <div className="overflow-x-auto rounded-lg border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
          <table className="min-w-full divide-y divide-slate-200 text-xs dark:divide-slate-800">
            <thead className="sticky top-0 z-10 bg-slate-50 dark:bg-slate-800">
              <tr>
                <th className="px-3 py-2 text-left font-medium text-slate-500 dark:text-slate-400">Vendedor</th>
                <th className="px-3 py-2 text-left font-medium text-slate-500 dark:text-slate-400">Plaza</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {rows.map((row) => (
                <tr key={row.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/60">
                  <td className="whitespace-nowrap px-3 py-1.5 text-slate-700 dark:text-slate-300">{row.vendedor}</td>
                  <td className="whitespace-nowrap px-3 py-1.5 text-slate-700 dark:text-slate-300">{row.plaza}</td>
                  <td className="whitespace-nowrap px-3 py-1.5">
                    <div className="flex items-center gap-3">
                      <Link
                        href={`/catalogos/vendedores/${row.id}`}
                        className="text-sm font-medium text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-slate-50"
                      >
                        Editar
                      </Link>
                      <CatalogoDeleteButton id={row.id} onDelete={deleteVendedor} />
                    </div>
                  </td>
                </tr>
              ))}
