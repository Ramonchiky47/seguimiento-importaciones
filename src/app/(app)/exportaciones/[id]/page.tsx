...
export default async function EditarExportacionPage({
  params,
}: {
  ...
  const row = data as Exportacion;
  row.dias_demoras = calcularDiasDemoras(row.ultimo_dia_libre_demoras);
  const boundUpdate = updateExportacion.bind(null, row.id);
  ...
  const { data: auditoriaData } = await supabase
    .from("auditoria_cambios_exportaciones")
      ...
      <header className="border-b border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
          ...
          {myPermissions.puede_exportar && (
            <a
              href={`/exportaciones/${row.id}/export-pdf`}
