...
import { DATE_FIELDS, TEXT_FIELDS } from "@/types/exportacion";
import {
  buscarBookingConSesion,
  listarReferenciasCargolinkExportacionPorRango,
  loginCargolink,
  mapCargolinkBookingToExportacion,
  normalizeFecha,
...
// sincronización masiva, que enriquece cada registro insertado en vez de
// dejarlo con el set reducido de campos).
async function enriquecerExportacionDesdeCargolink(
  supabase: Awaited<ReturnType<typeof createClient>>,
  ...
  const payload = mapCargolinkBookingToExportacion(cargolinkBooking, noBooking);
...
export async function createExportacion(formData: FormData) {
  const supabase = await createClient();
  const payload = { ...buildPayload(formData), seguro: buildSeguro(formData) };

  const { error } = await supabase.from("seguimiento_exportaciones").insert(payload);
  ...
  redirect("/exportaciones");
}

export async function updateExportacion(id: number, formData: FormData) {
  const supabase = await createClient();
  const payload = { ...buildPayload(formData), seguro: buildSeguro(formData) };
...
export async function actualizarExportacion(id: number) {
  const supabase = await createClient();
  ...
  const resultado = await enriquecerExportacionDesdeCargolink(
    supabase,
    session,
    row.id,
    row.booking,
  ...
  revalidatePath("/exportaciones");
  revalidatePath(`/exportaciones/${id}`);
}
...
// Paso 1: trae del Concentrado de Cargolink las referencias FCLE/LCLE del
// rango de fechas, las compara contra lo ya cargado en
// seguimiento_exportaciones (sin distinguir mayúsculas/minúsculas) y regresa
// solo la diferencia YA MAPEADA, lista para mostrarse en la ventana de
// confirmación — no inserta nada todavía.
export async function previsualizarSincronizacion(): Promise<PrevisualizacionSincronizacion> {
  const referencias = await listarReferenciasCargolinkExportacionPorRango(
    SYNC_FECHA_DESDE,
    syncFechaHasta(),
  );
...
// previsualización, y luego enriquece cada registro insertado con los ~20
// campos completos (mismo mecanismo que el botón "Actualizar" de una fila),
// reutilizando una sola sesión de Cargolink para todo el lote. Deja registro
// en sincronizacion_cargolink_exportaciones para poder mostrar la hora de la
// última actualización arriba del listado.
export async function confirmarSincronizacion(
  ...
  await supabase.from("sincronizacion_cargolink_exportaciones").insert({
...
export async function obtenerUltimaSincronizacion(): Promise<string | null> {
  ...
  const { data } = await supabase
    .from("sincronizacion_cargolink_exportaciones")
    .select("ejecutado_en")
...
export async function addIncidencia(exportacionId: number, formData: FormData) {
  const supabase = await createClient();
  ...
  const { error } = await supabase
    ...
    .insert({ exportacion_id: exportacionId, texto });

  if (error) throw new Error(error.message);

  revalidatePath(`/exportaciones/${exportacionId}`);
}

export async function deleteIncidencia(exportacionId: number, incidenciaId: number) {
...
export async function deleteExportacion(id: number) {
  const myPermissions = await getMyPermissions();
  if (!myPermissions.puede_borrar) throw new Error("No tienes permiso para borrar.");
