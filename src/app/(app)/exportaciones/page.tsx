...
import type { Exportacion } from "@/types/exportacion";
import { FIELD_LABELS, LIST_COLUMNS } from "@/types/exportacion";

export const dynamic = "force-dynamic";
// "Actualizar referencias" enriquece cada registro insertado consultando
// Cargolink uno por uno (ver confirmarSincronizacion), lo que puede tardar
// bastante más que el resto de las acciones de esta página.
...
export default async function ExportacionesPage({
  ...
  const rows = (data ?? []) as Exportacion[];
  ...
  // Paginado porque la API recorta a 1000 filas por default.
  ...
  const exportHref = `/exportaciones/export${
    exportParams.toString() ? `?${exportParams.toString()}` : ""
  }`;

  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      <RealtimeRefresher table="seguimiento_exportaciones" />
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {rows.map((row) => {
                ...
                return (
                  <ClickableRow
                    key={row.id}
                    href={`/exportaciones/${row.id}`}
                    className={rowClass}
                  >
                    <UpdateCell id={row.id} onUpdate={actualizarExportacion} stickyBg={rowBg} />
                    {LIST_COLUMNS.map((field) => (
                      <td
                        key={field}
